...
# FIXME: should put these in a new json file for mainnet/testnet
bchd_nodes = [
...
INVALID_NON_SLP_UNEXPECTED_BURNS = 'non-slp transaction, includes valid slp inputs'
INVALID_NON_SLP_OK = 'non-slp transaction'
...
class SlpPreflightCheck:
    ...
    def query(tx, *, selected_slp_coins=[], amt_to_burn=0, cb=None):
        ...
        for node in bchd_nodes:
            try:
                reqresult = requests.post('https://' + node + '/v1/CheckSlpTransaction', json=query_json, timeout=1)
                ...
                if resp.get('is_valid'):             # this happens when bchd says all is good.
                    ...
                    break
                        ...
                        INVALID_NON_SLP_UNEXPECTED_BURNS: # this happens which non-slp transaction has slp inputs
                    resp['ok'] = False
                    break
                elif resp.get('invalid_reason') == \
                        INVALID_NON_SLP_OK:  # this happens when transaction is non-slp, with no burn problems
                    resp['ok'] = True
                    break
